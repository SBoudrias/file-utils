...
File.prototype.fromBase = function(subPath) {
  ...
  if (fquery.isPathAbsolute(subPath)) {
    return subPath;
  }
  return path.join( this.option('base'), subPath );
};
...
File.prototype.read = function(filepath, options) {
  ...
  if (!options) { options = {}; }
  var contents;
  this.log.write('Reading ' + filepath + '...');
  try {
...
File.prototype.readJSON = function(filepath, options) {
  ...
  var result;
  this.log.write('Parsing ' + filepath + '...');
  try {
...
File.prototype.delete = function(filepath, options) {
  ...
  if (!options.force) {
    if (fquery.isPathCwd(filepath)) {
      ...
      return false;
    } else if (!fquery.doesPathContain(this.fromBase(''), filepath)) {
      this.log.error();
