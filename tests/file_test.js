...
exports['file'] = {
  ...
  'read': function(test) {
    test.expect(5);
    test.strictEqual(file.read('tests/fixtures/utf8.txt'), this.string, 'file should be read as utf8 by default.');
    test.strictEqual(file.read('tests/fixtures/iso-8859-1.txt', {encoding: 'iso-8859-1'}), this.string, 'file should be read using the specified encoding.');
    ...
    test.strictEqual(file.read('tests/fixtures/iso-8859-1.txt'), this.string, 'changing the default encoding should work.');
    test.done();
  },
  ...
  'copy and process': function(test) {
    ...
    file.copy('tests/fixtures/utf8.txt', tmpfile.path, {
      process: function(src, filepath) {
        test.equal(filepath, 'tests/fixtures/utf8.txt', 'filepath should be passed in, as-specified.');
  ...
  'exists': function(test) {
    ...
    test.equal(file.exists('tests/fixtures/does/not/exist'), false, 'nonexistent files do not exist.');
    test.done();
  },
  'isLink': function(test) {
    test.expect(6);
    test.equals(file.isLink('tests/fixtures/octocat.png'), false, 'files are not links.');
    test.equals(file.isLink('tests/fixtures'), false, 'directories are not links.');
    test.ok(file.isLink(path.join(tmpdir.path, 'octocat.png')), 'file links are links.');
    test.ok(file.isLink(path.join(tmpdir.path, 'expand')), 'directory links are links.');
    test.ok(file.isLink(tmpdir.path, 'octocat.png'), 'should work for paths in parts.');
    test.equals(file.isLink('tests/fixtures/does/not/exist'), false, 'nonexistent files are not links.');
    test.done();
  },
  'isDir': function(test) {
    test.expect(6);
  ...
  'isFile': function(test) {
    ...
    test.equals(file.isFile('tests/fixtures/does/not/exist'), false, 'nonexistent files are not files.');
    test.done();
  },
  'isPathAbsolute': function(test) {
    ...
    test.ok(file.isPathAbsolute(path.resolve('tests/fixtures/a.js')), 'should return true');
    test.equal(file.isPathAbsolute('tests/fixtures/a.js'), false, 'should return false');
    test.done();
  },
  'arePathsEquivalent': function(test) {
    ...
    test.ok(file.arePathsEquivalent(process.cwd(), '.', './', 'tests/..'), 'should return true');
  ...
  'doesPathContain': function(test) {
    ...
    test.ok(file.doesPathContain(process.cwd(), 'test', 'tests/fixtures', 'lib'), 'should return true');
    test.done();
  },
